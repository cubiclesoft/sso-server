<?php
	// Blowfish implementation in pure PHP.  Stream encrypt/decrypt and odd key sizes are supported.
	...
	class Blowfish
	{
		private $ciphermode, $parray, $sbox, $initvector1, $initvector2, $data;

		private $orig_parray = array(
			0x243F6A88, 0x85A308D3, 0x13198A2E, 0x03707344, 0xA4093822, 0x299F31D0,
			0x082EFA98, 0xEC4E6C89, 0x452821E6, 0x38D01377, 0xBE5466CF, 0x34E90C6C,
			0xC0AC29B7, 0xC97C50DD, 0x3F84D5B5, 0xB5470917, 0x9216D5D9, 0x8979FB1B
		);

		private $orig_sbox = array(
			array(
		...
		static function McryptEncrypt($data, $key, $options = array())
		{
			...
			$mp = mcrypt_module_open(MCRYPT_BLOWFISH, "", $options["mode"], "");
		...
		static function CreateDataPacket($data, $key, $options = array())
		{
			...
			{
				if (!isset($options["bits"]))  $options["bits"] = strlen($key) * 8;
				if (!isset($options["mode"]))  $options["mode"] = "ECB";
				if ($options["mode"] == "CBC" && !isset($options["iv"]))  $options["iv"] = "";
			...
			if (isset($options["key2"]))
			{
				...
				{
					if (!isset($options["bits2"]))  $options["bits2"] = strlen($options["key2"]) * 8;
					if ($options["mode"] == "CBC" && !isset($options["iv"]))  $options["iv"] = "";
		...
		static function Hash($data, $rounds, $mintime)
		{
			...
			if (self::IsMcryptAvailable())
			{
				...
				$iv = str_repeat("\x00", mcrypt_enc_get_iv_size($mp));
				if (mcrypt_enc_get_key_size($mp) != 56)  return array("success" => false, "error" => "Unexpected response from PHP function.");
		...
		// Call functions in the order displayed here.  Class can be reused.

		// ECB = Electronic CodeBook mode (Basic)
		// CBC = Cipher Block Chaining mode (Slightly more secure)
		public function SetBlockMode($mode)
		{
			$mode = strtoupper($mode);
			if ($mode != "ECB" && $mode != "CBC")  return false;
		...
		// Sets the initialization vector.  Only used in non-ECB modes.
		public function SetInitVector($ivdata)
		{
			while (strlen($ivdata) < 8)  $ivdata .= "\x00";

			$this->initvector1 = $this->MemChunkTo32(substr($ivdata, 0, 4));
			$this->initvector2 = $this->MemChunkTo32(substr($ivdata, 4, 4));

			return true;
		}

		// $key is usually 32 to 448 bits in length.  Typically on 8-bit boundaries but does not have to be.
		// $bits specifies how many bits are in $key.
		public function SetKey($key, $bits)
		{
			if ((int)(($bits + 7) / 8) > strlen($key))  $bits = strlen($key) * 8;
			if ($bits > 448)  $bits = 448;

			// XOR the key with the parray 32 bits at a time.
			if (!$bits)  $this->parray = $this->orig_parray;
			else
			{
				$y = 0;
				$y2 = 0;
				...
				for ($x = 0; $x < 18; $x++)
				{
					...
					for ($z = 0; $z < 32; $z++)
					{
						if (ord(substr($key, $y2, 1)) & $mask)  $currnum = $this->LIM32($this->SHL32($currnum, 1) | 0x01);
						else  $currnum = $this->SHL32($currnum, 1);
						$y++;
						if ($y == $bits)
						{
							$y = 0;
					...
					$this->parray[$x] = $this->LIM32($this->orig_parray[$x] ^ $currnum);
				}
			}

			// Copy the S-Boxes.
			$this->sbox = $this->orig_sbox;

			// Encrypt subkey pairs.
			$tempmode = $this->ciphermode;
			$this->ciphermode = "ECB";
			$this->data = "";
			$cryptbuffer = "\x00\x00\x00\x00\x00\x00\x00\x00";
			for ($x = 0; $x < 18; $x += 2)
			{
				$this->AddData($cryptbuffer);
				$cryptbuffer = $this->Encrypt();
				$this->parray[$x] = $this->MemChunkTo32(substr($cryptbuffer, 0, 4));
				$this->parray[$x + 1] = $this->MemChunkTo32(substr($cryptbuffer, 4, 4));
			}
			...
			for ($x = 0; $x < 4; $x++)
			{
				for ($y = 0; $y < 256; $y += 2)
				{
					$this->AddData($cryptbuffer);
					$cryptbuffer = $this->Encrypt();
					$this->sbox[$x][$y] = $this->MemChunkTo32(substr($cryptbuffer, 0, 4));
					$this->sbox[$x][$y + 1] = $this->MemChunkTo32(substr($cryptbuffer, 4, 4));
				}
			}
			$this->ciphermode = $tempmode;
		...
		// Finalizes data to be encrypted.  Do not add more data after this.
		public function Finalize()
		{
			$x = strlen($this->data) % 8;
			if ($x)
			{
				for (; $x < 8; $x++)  $this->data .= "\x00";
			}

			return true;
		}

		// Encrypts data if there are enough bytes.
		public function Encrypt()
		{
			$result = "";
			while (strlen($this->data) >= 8)
			{
				// Extract the next eight bytes from the data stream.
				$half1 = $this->MemChunkTo32(substr($this->data, 0, 4));
				$half2 = $this->MemChunkTo32(substr($this->data, 4, 4));
				if ($this->ciphermode == "CBC")
				{
					$half1 = $this->LIM32($half1 ^ $this->initvector1);
					$half2 = $this->LIM32($half2 ^ $this->initvector2);
				}
				for ($y = 0; $y < 15; $y++)
				{
					$half1 = $this->LIM32($half1 ^ $this->parray[$y]);
					$half2 = $this->LIM32($this->BLOWFISH_FUNC_F($half1) ^ $half2);
					$swap = $half1;
					$half1 = $half2;
					$half2 = $swap;
				}
				$half1 = $this->LIM32($half1 ^ $this->parray[15]);
				$half2 = $this->LIM32($this->BLOWFISH_FUNC_F($half1) ^ $half2);
				$half2 = $this->LIM32($half2 ^ $this->parray[16]);
				$half1 = $this->LIM32($half1 ^ $this->parray[17]);

				if ($this->ciphermode == "CBC")
				{
					$this->initvector1 = $half1;
					$this->initvector2 = $half2;
				}

				$result .= $this->MemChunkFrom64($half1, $half2);
		...
		// Decrypts data if there are enough bytes.
		public function Decrypt()
		{
			...
			while (strlen($this->data) >= 8)
			{
				...
				for ($y = 15; $y; $y--)
				{
					...
					$half2 = $swap;
					$half2 = $this->LIM32($this->BLOWFISH_FUNC_F($half1) ^ $half2);
					$half1 = $this->LIM32($half1 ^ $this->parray[$y - 1]);
				}
				...
				if ($this->ciphermode == "CBC")
				{
					$half1 = $this->LIM32($half1 ^ $this->initvector1);
					$half2 = $this->LIM32($half2 ^ $this->initvector2);
					$this->initvector1 = $vector1;
					$this->initvector2 = $vector2;
				}
		...
		// Common function used by both encrypt and decrypt routines.
		// #define BLOWFISH_FUNC_F(sbox, half)   ((((sbox)[0][(((UInt32)(half)) >> 24)] + (sbox)[1][(((UInt32)(half)) >> 16) & 0xFF]) ^ (sbox)[2][(((UInt32)(half)) >> 8) & 0xFF]) + (sbox)[3][((UInt32)(half)) & 0xFF])
		private function BLOWFISH_FUNC_F($half)
		{
			return $this->LIM32($this->ADD32($this->ADD32($this->sbox[0][($half >> 24) & 0xFF], $this->sbox[1][($half >> 16) & 0xFF]) ^ $this->sbox[2][($half >> 8) & 0xFF], $this->sbox[3][$half & 0xFF]));
		}
		...
		private function MemChunkTo32($data)
		{
			return $this->LIM32((int)(ord(substr($data, 0, 1)) << 24) | (int)(ord(substr($data, 1, 1)) << 16) | (int)(ord(substr($data, 2, 1)) << 8) | (int)ord(substr($data, 3, 1)));
		}

		private function MemChunkFrom64($half1, $half2)
		{
			return chr(($half1 >> 24) & 0xFF) . chr(($half1 >> 16) & 0xFF) . chr(($half1 >> 8) & 0xFF) . chr($half1 & 0xFF) . chr(($half2 >> 24) & 0xFF) . chr(($half2 >> 16) & 0xFF) . chr(($half2 >> 8) & 0xFF) . chr($half2 & 0xFF);
		}
	}
?>